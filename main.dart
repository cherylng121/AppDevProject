...
Future<void> _addProgress() async {
  ...
  try {
    ...
    final studentData = _searchResults.firstWhere(
      (result) => result['username'] == _selectedStudentUsername,
      orElse: () => {},
    );

    if (studentData.isEmpty || !studentData.containsKey('id')) {
      throw Exception('Student ID not found. Please search and select the student again.');
    }

    final studentId = studentData['id'];

    // Parse and validate score one more time
    ...
    final normalizedGrade = _gradeController.text.trim().toUpperCase();
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('❌ Failed to add progress: ${e.toString()}'),
          backgroundColor: Colors.red,
